# server.py
# Flask Server for Stock Tracker - FIXED VERSION
# - Waits for initial data before responding on /api/analysis
# - Returns a clear "pending" status until first cycle completes
...
try:
    from stock_analyzer import StockAnalyzer  # NOTE: match your actual filename
...
# Default stock configuration (ticker -> target price)
stock_config = {
    "TSLA": 500.0,
    "AAPL": 250.0,
    "NVDA": 200.0,
...
# Cache of latest reports from analysis thread
...
def analyze_stocks():
    ...
    if not ANALYZER_AVAILABLE:
        print("Analyzer not available", file=sys.stderr)
        analysis_thread_running = False
        return
                ...
                try:
                    ...
                    if report:
                        with data_lock:
                            ...
                            prob = report.get("probability", {}).get("composite_probability", 0)
...
# --------------- Helper: wait for initial data -----------------

def wait_for_initial_data(timeout_seconds: int = 30) -> bool:
    """
    Block for up to timeout_seconds until at least one stock
    has been analyzed and cached. Returns True if data available,
    False if timeout elapsed.
    """
    deadline = time.time() + timeout_seconds
    while time.time() < deadline:
        with data_lock:
            if stock_data and last_update is not None:
                return True
        time.sleep(0.5)
    return False
...
def get_analysis():
    """
    Return stock data for frontend.
    FIX: Wait briefly for background thread to populate data so
    the first page load does not see an empty list.
    """
    # If analyzer is not available at all, return error
    if not ANALYZER_AVAILABLE:
        return jsonify({
            "status": "error",
    ...
    # Wait up to 30 seconds for first results
    data_ready = wait_for_initial_data(timeout_seconds=30)
    ...
    with data_lock:
        ...
        if not data_ready or num_stocks == 0:
            # Clear "pending" response the frontend can handle
            response_data = {
                "status": "pending",
                "message": "Analysis running, no completed stocks yet",
                "timestamp": datetime.now().isoformat(),
                "stocks": {},
            }
...
def update_config():
    ...
    try:
        ...
        with data_lock:
            ...
            # Remove any cached stocks no longer tracked
